...
x_strategy = arrays(dtype=float, shape=st.integers(min_value=1, max_value=100))
y_strategy = arrays(dtype=float, shape=st.integers(min_value=1, max_value=100))
cv_strategy = st.lists(st.tuples(st.lists(st.integers(min_value=0), min_size=1), st.lists(
    st.integers(min_value=0), min_size=1)), min_size=1)
...
train_predict_handler_strategy = st.builds(
    ...
    reversifyfn=st.none(),
    property_name=st.none(),
    testfoldnum=st.integers(),
...
def test_train_predict_invalid_combination(train_predict_handler):
    with pytest.raises(FileNotFoundError) as exc_info:
        ...
        # assert "File at not found" in str(exc_info.value)
...
def test_train_predict_invalid_input():
    ...
    invalid_handler = TrainPredictHandler(x=np.array([1, 2, 3]), y=np.array([4, 5, 6]),
                                          cv=[([0], [1])])

    # Assert that the train_predict operation raises the expected exception
    with pytest.raises(Exception):
        invalid_handler.train_predict()
...
def test_train_predict_empty_cv():
    ...
    with pytest.raises(ValidationError) as exc_info:
        empty_cv_handler = TrainPredictHandler(
            x=np.array([1, 2, 3]), y=np.array([4, 5, 6]), cv=[])

    # Assert that the validation error contains the expected error message
    assert exc_info.value.errors()[0]["msg"] == "CV is empty"


def test_train_predict_single_cv_fold():
    # Create an instance of TrainPredictHandler with a single CV fold
    single_cv_handler = TrainPredictHandler(x=np.array([1, 2, 3]), y=np.array(
        [4, 5, 6]), cv=[([0, 1], [2])])

    # Perform the train_predict operation
    result = single_cv_handler.train_predict()
...
def test_train_predict_with_reversifyfn():
    ...
    handler = TrainPredictHandler(x=np.array([1, 2, 3]), y=np.array([4, 5, 6]), cv=[
                                  ([0], [1])], reversifyfn=reversifyfn)
...
def test_train_predict_invalid_weight_bins():
    ...
    invalid_handler = TrainPredictHandler(
        x=np.array([1, 2, 3]), y=np.array([4, 5, 6]), cv=[([0], [1])], weight_bins=0
    )
...
def test_train_predict_with_x_noise():
    ...
    handler = TrainPredictHandler(
        x=np.array([1, 2, 3]), y=np.array([4, 5, 6]), cv=[([0], [1])], x_noise=np.array([0.1, 0.2, 0.3])
    )
