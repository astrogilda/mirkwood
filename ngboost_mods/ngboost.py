...
class NGBoost:
    ...
    def pred_param(self, X, max_iter=None):
        ...
        for i, (models, s, col_idx) in enumerate(
            ...
            if max_iter and i == max_iter:
                break
            resids = np.array([model.predict(X[:, col_idx])
    ...
    def partial_fit(
        ...
        if not train_loss_monitor:
            def train_loss_monitor(D, Y, W): return D.total_score(  # NOQA
                Y, sample_weight=W
            )

        if not val_loss_monitor:
            def val_loss_monitor(D, Y): return D.total_score(  # NOQA
        ...
        for itr in range(len(self.col_idxs), self.n_estimators + len(self.col_idxs)):
            ...
            if X_val is not None and Y_val is not None:
                val_params -= (
                    ...
                    * scale
                    * np.array(
                        [m.predict(X_val[:, col_idx])
                         for m in self.base_models[-1]]
                    ).T
                )
    ...
    def _get_feature_importance(self, tree, tree_index):
        ...
        total_feature_importance[self.col_idxs[tree_index]
